...
///////////////////////////// TEXTURE IMAGES ///////////////////////////////////////
	...
	Image bigMonster = new Image("images/monsterBig.jpg");
	...
	Image farmer = new Image("images/farmer.jpg");
////////////////////////////////////////////////////////////////////////////////	
		...
		for (int i = 0; i < cols; i++) {
			for (int j = 0; j < rows; j++){
				...
				rectGrid[i][j].setFill(new ImagePattern(wall));
					...
					buttonGrid[i][j].setOpacity(1);
					//rectGrid[i][j].setFill(Color.ORANGE);
					rectGrid[i][j].setFill(new ImagePattern(farmer));
				}else if (ch == '*' ){
					buttonGrid[i][j].setOpacity(1);
 ...
 // sets up the walls and items, doors and monsters
	...
	*/
 	public void setBoard( Maze m){
 		...
 		m.setRoomItems(1,0,false,true,false,false,false,false,false,false,false,false,false,false); // has door
 		// room (2,0)
 		m.setRoomWalls(2,0,false,false,true,true);
 		// room (3,0)
 		m.setRoomWalls(3,0,true,true,true,false);
 		m.setRoomItems(3,0,false,false,false,false,false,false,false,true,false,false,false,false); // has comboLock
 		// room (1,0)
 		m.setRoomWalls(0,1,true,true,false,false);
 		m.setRoomItems(0,1,false,false,false,false,false,false,true,false,false,false,false,false); // has hint at (0,1)
 		// room (1,1)
 		m.setRoomWalls(1,1,true,false,true,false); 
 		// room (2,1)
 		m.setRoomWalls(2,1,false,false,true,true);
 		m.setRoomItems(2,1,false,false,false,false,false,false,true,false,false,false,false,false); // has hint at (2,1)
 		// room (3,1)
 		m.setRoomWalls(3,1,false,true,false,false);
 		// room (0,2)
 		m.setRoomWalls(0,2,true,true,false,true);
 		m.setRoomItems(0,2,false,false,false,false,false,true,false,false,false,false,false,false); // has riddle
 		// room (1,2)
 		m.setRoomWalls(1,2,false,true,false,false);
 		// room (2,2)
 		m.setRoomWalls(2,2,true,true,true,false);
 		m.setRoomItems(2,2,true,false,false,false,false,false,false,false,false,false,false,false); // has key
 		// room (3,2)
 		m.setRoomWalls(3,2,true,true,false, false);
 		// room (0,3)
 		m.setRoomWalls(0,3,true,false,true,true);
 		m.setRoomItems(0,3,false,false,true,false,false,false,false,false,false,false,false,false); // has map
 		// room (1,3)
 		m.setRoomWalls(1,3,false,true,false,true);
 		// room (3,2)
 		m.setRoomWalls(2,3,true,false,true,true);
 		m.setRoomItems(2,3,false,false,false,false,false,true,false,false,false,false,false,false); //place the riddle in the room below the key
 		// room (3,3)
		...
		halfWay = new Point(1,4);
 		...
 		m.setRoomItems(2,0,false,false,false,false,false,false,false,false,true,false,false,false); // place goat
 		m.setRoomWalls(3,0,true,false,true,false);
 		m.setRoomWalls(4,0,false,true,true,true);
 		m.setRoomItems(4,0,false,false,false,false,false,true,false,false,false,false,false,false); // riddle
 		...
 		m.setRoomItems(5,1,false,false,false,false,false,false,false,false,false,true,false,false); // place wolf
 		// ROW 2
 		...
 		m.setRoomItems(1,3,false,false,false,false,false,false,false,false,false,false,false,true); // place farmer
 		...
 		m.setRoomItems(3,4,false,false,false,false,false,false,false,false,false,false,true,false); // place cabbage
 		...
 		m.setRoomItems(0,5,false,true,false,false,false,false,false,false,false,false,false,false);
 		m.setDoorLocation(0,5);  // change the door location 
 		// place the key in specific location
 		m.setRoomItems(5,0,true,false,false,false,false,false,false,false,false,false,false,false);
	...
	*/
	public void inputControl()  {
		...
		if ( hitPlay == true && currentPosition.isEqual(firstRiddle) && storeInput.equalsIgnoreCase(theRiddle.getAnswer()) ) {
			...
			gameBoard.setRoomItems(firstRiddle.getXCoordinate(),firstRiddle.getYCoordinate(),tempRoom.getHasKey(),tempRoom.getHasDoor(),tempRoom.getHasMap(),tempRoom.getHasMonster(),tempRoom.getHasFood(),false,tempRoom.getHasHint(),tempRoom.getHasComboLock(),tempRoom.getHasGoat(),tempRoom.getHasWolf(),tempRoom.getHasCabbage(),tempRoom.getHasfarmer()); // has riddle
		...
		if( hitPlay == true && currentPosition.isEqual(secondRiddle) && storeInput.equalsIgnoreCase(theRiddle.getAnswer()) ) {
			...
			gameBoard.setRoomItems(secondRiddle.getXCoordinate(),secondRiddle.getYCoordinate(),tempRoom.getHasKey(),tempRoom.getHasDoor(),tempRoom.getHasMap(),tempRoom.getHasMonster(),tempRoom.getHasFood(),false,tempRoom.getHasHint(),tempRoom.getHasComboLock(),tempRoom.getHasGoat(),tempRoom.getHasWolf(),tempRoom.getHasCabbage(),tempRoom.getHasfarmer()); // has riddle
		...
		if( hitPlay == true && currentPosition.isEqual(thirdRiddle) && storeInput.equalsIgnoreCase(theRiddle.getAnswer()) ) {
			...
			gameBoard.setRoomItems(thirdRiddle.getXCoordinate(),thirdRiddle.getYCoordinate(),tempRoom.getHasKey(),tempRoom.getHasDoor(),tempRoom.getHasMap(),tempRoom.getHasMonster(),tempRoom.getHasFood(),false,tempRoom.getHasHint(),tempRoom.getHasComboLock(),tempRoom.getHasGoat(),tempRoom.getHasWolf(),tempRoom.getHasCabbage(),tempRoom.getHasfarmer()); // has riddle
			//setRoomItems(int x , int y,  boolean key, boolean door ,boolean map, boolean monster, boolean food, boolean riddle, boolean hint, boolean comboLock, boolean goat, boolean wolf, boolean cabbage )		
		...
		//start looking at hints
			...
			sayThis = instructions.room1Instructions();
			bigText.setText(sayThis);
			hitLook = true;
		}	
			
		if (hitLook == true && (storeInput.equalsIgnoreCase("shelf")) && currentPosition.isEqual(firstHint)) {
			sayThis = instructions.shelfInstructions();
			bigText.setText(sayThis);
			hitLook = false;
		}	
		
		if (hitLook == true && (storeInput.equalsIgnoreCase("bear")) && currentPosition.isEqual(firstHint)) {
			if (tempHero.getHasLeverOne() == false) {
				sayThis = instructions.bearInstructionsNoLever();
				bigText.setText(sayThis);
				gameBoard.takeLeverOne();
				postItems();
			}
			else if (tempHero.getHasLeverOne() == true){
				sayThis = instructions.bearInstructionsYesLever();
				bigText.setText(sayThis);
			}
			hitLook = false;
		}
		
		if (hitLook == true && (storeInput.equalsIgnoreCase("puddle")) && currentPosition.isEqual(firstHint)) {
			sayThis = instructions.puddleInstructions();
			bigText.setText(sayThis);
			hitLook = false;
		}
		
		if (hitLook == true && (storeInput.equalsIgnoreCase("blackboard")) && currentPosition.isEqual(firstHint)) {
			sayThis = instructions.blackboardInstructions();
			bigText.setText(sayThis);
			hitLook = false;
		}
 ...
 //////////////////////////////// METHODS THE REQUIRE REPLACEMENT/TRANSLATION //////////////////////////////////	
	...
	*/
	public void printMap(){
		...
		image = new Image("images/mapTwo.jpg");
		}
	...
	*/
	public void fightMonster(Maze gameBoard){	
		...
		if (  gameBoard.getMonster().isAlive() && gameBoard.getHero().isAlive() ){
			...
			drawImage(gcL,bigMonster);  // bring up the picture of the monster
