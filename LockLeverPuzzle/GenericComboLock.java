 ...
 * 
 *          CLASS GENERIC COMBINATION LOCK The number of combo switches is a
 *          maximum of 3. When user combo input matches key, combo lock will
 *          unlock to true.
 */
...
////////////////////Other Methods//////////////////////
	/**
	 * Checks if the user input matches the combination lock digits theoretical
	 * restriction from 0-9 inclusive
	 * 
	 * @param comboNum the number that has been entered by the player.
	 * @return A true or false; true meaning that the number will be set, false
	 *         meaning that the number will be ignored.
	 */
	...
	/**
	 * Checks if the input for The Key matches the combination lock digits
	 * theoretical restriction from 0-9 inclusive
	 * 
	 * @param comboNum the number that has been entered by the player.
	 * @return A true or false; true meaning that the number will be set, false
	 *         meaning that the number will be ignored.
	...
	/**
	 * If all comboNum# matches the comboNum#_Key, checkCombo is a Pass If it does
	 * not, the user modifiers comboNum# will be all set back to 0
	 * 
	...
	public boolean checkCombo() {
		...
		if ((comboNum1_Key == comboNum1) && (comboNum2_Key == comboNum2) && (comboNum3_Key == comboNum3)) {
			comboMatch = true;
			System.out.println("The lock is now unlocked. "); // MIGHT NEED TO EDIT:unknown and will return in a diff
																// method(easier for overriding)
		} else {
	...
	// Generic User Click Description(PUT IN A NEW CLASS FOR ITEMS LATER ON; this
	// one is an override of the generic static objects)
	/**
	 * Provides information about the combination lock and checks the status of the
	 * lock being locked or unlocked.
	 */
