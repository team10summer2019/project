 ...
 * 
 *          CLASS LOCK LEVER Lever comes in two pieces. This class checks if the
 *          two lever pieces are in the player inventory or not, then combine it
 *          if it exists LockLever works with dynamicObjects as well as
 *          LockLeverPuzzle
 */
public class LockLever {
	/*
	 * CLASS LOCK LEVER Lever comes in two pieces. This class checks if the two
	 * lever pieces are in the player inventory or not, then combine it if it exists
	...
	// METHODS
	/**
	 * Get leverPiece1 and leverPiece2 info from player inventory If both pieces
	 * exist in the inventory, puts together lever
	 * 
	 * @return
	 */
	public boolean combinedLeverStatus() {
		if (obj_LeverPiece1 == true && obj_LeverPiece2 == true) {// CHANGE THIS INFO SO THAT IT CHECKS THE PLAYERS
																	// INVENTORY
			obj_Lever_Key = true;
	...
	/**
	 * Checks if both objects exists "Makes" and adds lockLever to inventory, while
	 * removing the pieces.
	 * 
	 * @param i
	 */
	public void addCombinedLever(Inventory i) {
		if (combinedLeverStatus() == true) {
			...
			System.out.println(
					"Having both items,'leverPiece1' and 'leverPiece2' was attached together to create item 'lockLever'. Now you have a fully functioning lever piece.");
		}
	}

	/**
	 * If both pieces exist a new string will be prompted for the lever pieces to be
	 * combined(unused)
	 */
